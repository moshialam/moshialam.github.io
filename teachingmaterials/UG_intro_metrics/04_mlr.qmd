...
# Partialing out {background-color="#1c5253"}
...
- Suppose instead we only estimate the SLR of $y$ on $x_{1i}$:
  $$\tilde{y_i} = \tilde{\beta}_0 + \tilde{\beta}_1 x_{1i}$$

- SLR estimate $\tilde{\beta}_1$ relates to the MLR estimate $\hat{\beta}_1$ via:
...
3. Thus generically in a MLR of $y_i$ on $x_{1i}, \cdots, x_{ki}$, the OLS estimate on the coefficient on $x_{1i}$ 
...
- We will start with the homoskedasticity assumption since it helps set some fundamental intuition.
