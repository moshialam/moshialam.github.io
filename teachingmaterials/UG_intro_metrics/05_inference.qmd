...
- **Central Limit Theorem (CLT):** Let $\{X_1, X_2, \ldots, X_n\}$ be a random sample of size $n$ from a population with mean $\mu$ and variance $\sigma^2$. Then, as $n$ approaches infinity, the sampling distribution of the sample mean $\bar{X}$ approaches a normal distribution with mean $\mu$ and variance $\sigma^2/n$. 
  - i.e. as sample size goes to infinity, $\bar{X} \sim N(\mu, \sigma^2/n)$ so that $\frac{\bar{X} - \mu}{\sigma/\sqrt{n}} \sim N(0,1)$
...
## {.smaller}
- **p-value:** The probability of observing the sample statistic or more extreme values under the $H_0$
...
The population error $u_i$ is normally distributed with mean 0 and constant variance $\sigma^2$ for all $i = 1, 2, \ldots, n$ and independent of the regressors.
$$u_i \sim N(0, \sigma^2)$$

- Much stronger assumption than others
- This directly implies MLR.4 and MLR.5
- The full set of assumptions MLR.1-MLR.6 are called the classical linear model (CLM) assumptions. They imply: 
$$y \mid x_1, \ldots x_k \sim N(\beta_0 + \beta_1 x_{1i} + \ldots + \beta_k x_{ki}, \sigma^2)$$

## Arguements for and against the normality assumption {.smaller}
...
Under MLR.1-MLR.6, the OLS estimators $\hat{\beta}_0, \hat{\beta}_1, \ldots, \hat{\beta}_k$ are normally distributed.
...
$$
\frac{\hat{\beta}_j-\beta_j}{\operatorname{sd}\left(\hat{\beta}_j\right)} \sim \operatorname{Normal}(0,1)
$$

Proof: See Math Refresher B. Note that each $\hat{\beta_j}$ can be written as a linear combination of the $u_i$'s and the $x_{ji}$'s. Specifically, $\hat{\beta}_j = \beta_j + \sum_{i=1}^n w_{ij} u_i$, where the $w_{ij} = \hat{r_{ij}}/SSR_j$  where $\hat{r_{ij}}$ is the $i$th residual from regressing $x_j$ on all other independent variables and $SSR_j$ is the sum of squared residuals from that regression.
...
$$
\frac{\left(\hat{\boldsymbol{\beta}}_j-\beta_j\right)}{\operatorname{se}\left(\hat{\boldsymbol{\beta}}_j\right)} \sim t_{n-k-1} \quad \text { for } j=0,1, \ldots, k
$$

where $k+1$ is the number of unknown parameters in the population model and $n-k-1$ is the degrees of freedom (df). 


 The standard error of $\hat{\beta}_j$ is $\operatorname{se}\left(\hat{\beta}_j\right)=\sqrt{\operatorname{Var}\left(\hat{\beta}_j\right)}$ that we derived in the last class. 

 Now that we know the sampling distribution of $\hat{\beta}_j$, we can make inferences about the population parameter $\beta_j$ under any given  null hypothesis 
...
- set b= 0. Test statistic: $t = \frac{\hat{\beta}_j - b}{\operatorname{se}(\hat{\beta}_j)} = \frac{\hat{\beta}_j}{\operatorname{se}(\hat{\beta}_j)}$


## One-sided alternative {.smaller}
- **One-sided alternative:** $H_1: \beta_j > 0$ or $H_1: \beta_j < 0$

## Two-sided alternative {.smaller}
- **Two-sided alternative:** $H_1: \beta_j \neq 0$
...
## Testing Single Linear combination of parameters {.smaller}

## Testing Multiple Linear Restrictions {.smaller}
### F-test {.smaller}
