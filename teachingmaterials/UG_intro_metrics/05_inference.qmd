...
- Based on d.o.f. we can look up the critical value $t_{\alpha, n-k-1}^*$ and compare with $t_{\hat{\theta}}$
  - Reject $H_0$ if $t_{\hat{\theta}} < t_{\alpha, n-k-1}^*$
  - p-value = $P(T < t_{\hat{\theta}})$ where $T \sim t_{n-k-1}$
  - Confidence interval: $\hat{\theta} \pm t_{\alpha, n-k-1}^* \times \operatorname{se}(\hat{\theta})$
- Easy to calculate if $\widehat{\operatorname{cov}(\hat{\beta}_1, \hat{\beta}_2)}$ is known 
...
library(wooldridge)
data <- twoyear
data$total_years <- data$univ + data$jc
summary(lm(lwage ~ jc + total_years + exper, data = data))
```
